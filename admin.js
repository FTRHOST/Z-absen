...
function renderKantor() {
    ...
    filtered.forEach(kantor => {
        ...
        const argsStr = `'${kantor.id}', '${kantor.nama}', '${kantor.latitude}', '${kantor.longitude}', '${kantor.radius}', '${kantor.jam_masuk || '08:00:00'}', '${kantor.jam_keluar || '17:00:00'}', '${kantor.jam_mulai_istirahat || '12:00:00'}', '${kantor.jam_selesai_istirahat || '13:00:00'}'`;
                ...
                <div class="card shadow-sm h-100 border-0 dashboard-card-hover" style="border-radius: 12px; transition: transform 0.2s;">
                    <div class="card-body">
                        ...
                        <p class="card-text small text-muted mb-2">
                            <i class="fas fa-map-marker-alt me-2 text-danger"></i>${kantor.latitude}, ${kantor.longitude} (Rad: ${kantor.radius}m)
                        </p>
        ...
        tableBody.innerHTML += `
            <tr>
                ...
                <td>${kantor.latitude}, ${kantor.longitude} <br> <span class="badge bg-secondary">Rad: ${kantor.radius}m</span></td>
...
async function hapusKantor(id) {
    ...
    Swal.fire("Terhapus", "Data kantor berhasil dihapus.", "success");
    loadDataKantor();
}
...
function showDetailAbsensi(tanggal, dateStr) {
    ...
    records.forEach(absen => {
        let hasFoto = absen.foto_url || absen.foto_istirahat_keluar || absen.foto_istirahat_masuk || absen.foto_keluar;
...
async function exportCsvHarian(tanggal) {
    try {
        ...
        let header = "Tanggal,Nama,Cabang,Waktu Masuk,Waktu Mulai Istirahat,Waktu Selesai Istirahat,Waktu Pulang,Status Kehadiran,Jarak (Meter)";
        if (includeMedia) {
        ...
        if (includeMedia) {
            ...
            try {
                ...
                for (const row of records) {
                    ...
                    await fetchFoto(row.foto_url, userFolder, `${tanggal}_Masuk.png`);
...
async function setujuiSemuaCuti(bulan) {
    ...
    try {
        ...
        // Supabase bulk update isn't straightforward without an RPC, so loop it
        for (const id of ids) {
        ...
        bootstrap.Modal.getInstance(document.getElementById('modalDetailCuti')).hide();
        loadDataCuti();
    ...
    try {
        ...
        if (data) {
            ...
            if (data.nama_aplikasi) {
                ...
                if (brand) brand.innerHTML = `⚙️ Admin Panel ${data.nama_aplikasi}`;
            }
        }
    } catch (err) {
        console.error(err);
